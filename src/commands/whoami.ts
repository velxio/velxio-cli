...
import { fmtMinutes, type Output, type Renderer } from '../ui/types.ts';
...
export const WHOAMI_PATH = '/api/pro/ci/whoami';

/** The identity block: `whoami` is only this, and `login` ends with it. */
export function renderIdentity(r: Renderer, server: string, token: string, body: WhoamiBody): void {
  if (r.json) {
